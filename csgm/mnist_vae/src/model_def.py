...
class Hparams(object):
    def __init__(self):
        self.n_hidden_recog_1 = 1000  # 1st layer encoder neurons
        self.n_hidden_recog_2 = 1000  # 2nd layer encoder neurons
        self.n_hidden_gener_1 = 1000  # 1st layer decoder neurons
        self.n_hidden_gener_2 = 1000  # 2nd layer decoder neurons
        self.n_input = 2048           # CHANNEL data input (img shape: 60*64)
        self.n_z = 100                # dimensionality of latent space
...
#    reconstr_losses = tf.reduce_sum(tf.nn.sigmoid_cross_entropy_with_logits(labels=x, logits=logits), 1)
    reconstr_losses = tf.reduce_mean(tf.square(x-logits))

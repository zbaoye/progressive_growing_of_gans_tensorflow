...
def dcgan_discrim(x_hat_batch, hparams):
    ...
    x_hat_image = tf.reshape(x_hat_batch, [-1, 64, 64, 2])
    all_zeros = tf.zeros([64, 64, 64, 2])
...
def dcgan_gen(z, hparams):
    ...
    x_hat_batch = tf.reshape(x_hat_full[:hparams.batch_size], [hparams.batch_size, 64*64*2])
